	...
	<div>
		...
		<div class="flex flex-col gap-6 ml-4 items-center">
			...
			<div class="grid grid-cols-2 gap-4 mx-auto">
				<p>Name: {{ searchedNdc.generic_name.toTitle() }}</p>
				<p>Brand: {{ searchedNdc.brand_name.toLowerCase().toTitle() }}</p>
				<p>Manufacturer: {{ searchedNdc.labeler_name.toTitle() }}</p>
			...
			<button class="btn w-1/4 ml-auto mr-6" @click="saveNdc">Save NDC</button>
		</div>
	</div>
</template>
...
import { OpenNDC, NDC } from "~~/types/types";
...
const saveNdc = () => {
	...
	if (formIsValid.value) {
		...
		const ndc: NDC = {
			...
			name: searchedNdc.brand_name,
			genericName: searchedNdc.generic_name,
			manufacturer: searchedNdc.labeler_name,
			doseForm: searchedNdc.dosage_form,
			date_added: new Date(),
