<template>
	<div>
		<h2 class="header">Add/Edit Doctor</h2>
		<div class="flex flex-col gap-4 ml-4">
			...
			<div class="flex justify-between my-4">
				<button class="w-1/4 mr-4 reverse" @click="deleteDoctor">
					Delete Doctor
				</button>
				<button class="w-1/4 mr-4" @click="editDoctor">Save Doctor</button>
			</div>
		</div>
	</div>
</template>
...
const selectedDoctor = doctorsStore.selectedDoctor;
...
const setInputs = () => {
	for (let key in input) {
		input[key].val = selectedDoctor[key];
	}
};
...
const deleteDoctor = async () => {
	uiStore.toggleFunctionLoading(true);
	await doctorsStore.deleteDoctor(selectedDoctor.doctor_id);
	clearInputs();
	router.push("/doctors");
	uiStore.toggleFunctionLoading(false);
};
